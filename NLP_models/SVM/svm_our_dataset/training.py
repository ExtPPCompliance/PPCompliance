...
Corpus = pd.read_csv("../policy_all_in_one_filter_purged.csv", encoding='utf-8')
...
Corpus['text_final'] = Corpus['review'].map(text_preprocessing)
...
report =classification_report(predictions_NB, Test_Y, output_dict=True)
df_report = pd.DataFrame(report).transpose()
df_report.to_csv('our_our_report_nb.csv',index=True)
...
report =classification_report(predictions_SVM, Test_Y, output_dict=True)
df_report = pd.DataFrame(report).transpose()
df_report.to_csv('our_our_report_svm.csv',index=True)

# Saving Encdoer, TFIDF Vectorizer and the trained model for future infrerencing/prediction

# saving encoder to disk
filename = 'result/labelencoder_fitted.pkl'
...
filename = 'result/svm_trained_model.sav'
pickle.dump(SVM, open(filename, 'wb'))

filename = 'result/nb_trained_model.sav'
